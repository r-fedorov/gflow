...
class Buffer:
    ...
    def add(self, states, trajs, rewards, it, buffer="main", criterion="greater"):
        ...
        if buffer == "main":
            ...
            raise ValueError(f"Unknown buffer identifier. Received {buffer}, expected main or replay")
    ...
    def _add_greater(self, states, trajs, rewards, it, allow_duplicate_states=False):
        ...
        for idx, (state, traj, reward) in enumerate(zip(states, trajs, rewards)):
            ...
            if (
                ...
                self.replay.sort_values(
                    by="reward", ascending=False, inplace=True)
        self.save_replay()
        return self.replay
    ...
    def make_data_set(self, config):
        ...
        if "type" not in config:
            ...
            with open(config.path, "rb") as f:
                ...
                print(f"The data set contains {n_samples_orig} samples",
                      end="")
                samples = self.env.process_data_set(samples)
                n_samples_new = len(samples)
                if n_samples_new != n_samples_orig:
                    print(f", but only {n_samples_new} are valid according to the environment settings. Invalid samples have been discarded.")
            ...
            # Process the raw samples into the internal state representation.
            samples = self.env.process_data_set(samples)

            # Check if energies_column is specified.
            if "energies_column" in config:
                energies = df[config.energies_column].tolist()
                print(f"ive got all the energies", end="")
            else:
                # Fall back to computing energies via proxy if the energies_column is missing.
                print(f"no energies are found, recomputing proxy", end="")
                energies = self.proxy(self.env.states2proxy(samples)).tolist()

            n_samples_orig = len(samples)
            print(f"The data set contains {n_samples_orig} samples", end="")
            ...
            samples = self.env.get_uniform_terminating_states(
        ...
        # Create a pandas DataFrame for logging/record purposes.
        ...
        return df_data, {"x": samples, "energy": energies}
    ...
    def select(
        ...
        if mode == "permutation":
            ...
            if "rewards" in data_dict:
                ...
                raise ValueError(
                    f"Data set does not contain reward or energy key.")
